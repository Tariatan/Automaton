...
- Note: grayscale is the correct tool for locating the playfield boundary itself (the corner markers), which is a separate concern from cluster detection and works well there.
...
- Use `*.template.masked.png` as the ground-truth mask for polygon extraction. Each sample must have a corresponding masked file (white regions on dark background).
- The known-sample template cache assumes the `templates` folder is stable for the process lifetime. That leaves a bounded unmanaged `Mat` retention concern, but it is acceptable for the current fixed-runtime model.
