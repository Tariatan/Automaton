...
internal sealed class TemplateButtonDetector : IDisposable
{
    ...
    public bool Detect(string imagePath, out TemplateButtonLocation location)
    {
        ...
        return Detect(image, out location);
    }

    public bool Detect(Mat screen, out TemplateButtonLocation location)
    {
        location = default;
        if (screen.Empty())
        {
        ...
        TemplateButtonLocation? bestLocation = null;
        foreach (var scale in TemplateScales)
        {
            ...
            try
            {
                ...
                {
                    bestLocation = new TemplateButtonLocation(bounds, score);
                }
...
internal readonly record struct TemplateButtonLocation(Rect Bounds, double Score);
